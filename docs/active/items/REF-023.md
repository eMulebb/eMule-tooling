...
Two formatting cleanup slices have landed on `main`:

Commit `1c4eeee` (`REF-023: remove MiniUPnP port sprintfs`):
...
Commit `8c5b509` (`REF-023: remove debug endpoint sprintfs`):
...
Latest scoped validation for the current slice:

- `workspace.ps1 validate`
- `workspace.ps1 build-app -Config Debug -Platform x64`
  - First pass rebuilt the main app and surfaced existing project-wide warning
    debt in unchanged files.
  - Immediate incremental pass completed all app targets with `0` warnings.

The item remains `In Progress`: current `main` still has remaining live
`_stprintf` / `sprintf` sites in `WebServer.cpp` and `SendMail.cpp`, plus
commented references in `EmuleDlg.cpp` and `Parser.cpp`.
