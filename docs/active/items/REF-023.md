...
The MiniUPnP port-formatting slice landed on `main` in commit `1c4eeee`
(`REF-023: remove MiniUPnP port sprintfs`):
...
Latest scoped validation for this slice:
...
`_stprintf` / `sprintf` sites in `WebServer.cpp`, `OtherFunctions.cpp`, and
`SendMail.cpp`, plus one commented `wsprintf` debug line in `EmuleDlg.cpp`.
