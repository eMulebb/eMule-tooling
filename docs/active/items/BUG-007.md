...
- [ ] `RemoveAll` leaves `m_pTail` in the same one-past-end empty sentinel state used after construction
- [ ] `RemoveHead` resets the ring to that same empty sentinel state when the last element is removed
- [ ] `operator[]` has `ASSERT(index < m_nCount)` guard
- [ ] `Head()` and `Tail()` assert on empty logical access
