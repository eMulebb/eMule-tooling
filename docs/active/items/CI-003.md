...
- Policy audit state:
  `repos\eMule-tooling\ci\check-build-policy.ps1` asserts those app properties
  for both configurations, and routine `workspace.ps1 validate` runs that
  audit.
- Build orchestration state: `repos\eMule-build\workspace.ps1` runs
...
- 2026-05-10 verification:
  `workspace.ps1 build-app -Config Debug -Platform x64` passed with `APP main`
  and `APP main CFG` both OK. Build log:
  `workspaces\v0.72a\state\build-logs\20260510-062221\summary.json`.
- 2026-05-10 verification:
  `workspace.ps1 build-app -Config Release -Platform x64` passed with
