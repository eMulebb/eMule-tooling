...
|----|----------|------|---------|
| [BUG-069](../items/BUG-069.md) | Major | WebServer/static files | Static resource serving does not prove final path containment and reads whole files into memory. |
| [BUG-070](../items/BUG-070.md) | Minor | helper threads/shutdown | Several helper thread constructors ignore `AfxBeginThread()` failure before shutdown waits. |
| [BUG-071](../items/BUG-071.md) | Major | server list persistence | `server.met` save and auto-update still use destructive backup/promotion moves. |
| [BUG-072](../items/BUG-072.md) | Minor | Kad persistence | `preferencesKad.dat` and `nodes.dat` still save directly to live files. |
| [BUG-073](../items/BUG-073.md) | Major | WebServer/session concurrency | Session and bad-login arrays are mutated from concurrent request threads without synchronization. |
| [BUG-074](../items/BUG-074.md) | Minor | archive preview threading | Archive preview scanner uses volatile cancellation and synchronous worker-to-UI result handoff. |
...
1. Fix [BUG-069](../items/BUG-069.md) and [BUG-073](../items/BUG-073.md) before expanding the
   WebServer/REST surface further.
2. Fix [BUG-071](../items/BUG-071.md) and [BUG-072](../items/BUG-072.md) as the next persistence
   durability slice, reusing the current atomic-promotion helpers where possible.
3. Fix [BUG-070](../items/BUG-070.md) when touching upload/disk helper threads or when
   adding low-resource startup/shutdown tests.
4. Resolve [BUG-074](../items/BUG-074.md) together with the archive-preview retain/remove
