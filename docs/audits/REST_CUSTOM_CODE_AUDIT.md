...
- qBittorrent compatibility `properties` and `files` routes now parse the
  required `hash` query parameter in the qBit seam, reusing the shared query
  parser and native eD2K hash shape check before controller dispatch.
...
- Native server create/update, URL import, and Kad bootstrap bodies now
  validate public text, boolean, and TCP-port shape in the shared route seam
  before command dispatch.
- Native app-preferences PATCH bodies now validate mutable preference names,
  numeric bounds, boolean fields, and empty-patch rejection in the shared route
  seam before preference persistence runs.
- Native search-start request bodies now validate query text, method/type
  vocabulary, optional size ranges, availability bounds, extension type, and
  `clearExisting` shape in the shared route seam before command dispatch. The
  command parser delegates the default method token and vocabulary checks to
  the same route helpers.
- Native category create/update and friend-create bodies now validate public
  field shape in the shared route seam before command dispatch. Category path
  shareability and category/friend existence checks remain in the live command
  layer because they depend on current preferences and runtime state.
...
| qBit form/query parsing | `WebServerQBitCompatSeams::TryValidateFormRequestMetadata`, `TryParseFormBody`, qBit category request parsers, qBit category filters, and qBit required hash query parsing | Kept with reason | qBittorrent compatibility needs `application/x-www-form-urlencoded` decoding with unique decoded field names and adapter-specific error text. The implementation now rejects non-form `Content-Type` values before parsing non-empty POST bodies, delegates decoding to the shared REST URL-encoded field parser, and routes category add/set/create/filter plus properties/files hash requests through shared native text/hash policy; tests cover metadata rejection, duplicate fields/query parameters, empty names, malformed escapes, required field checks, trimming, control-character rejection, and malformed hash selectors. |
...
| Server and Kad body validation | Native server create/update, URL imports, and Kad bootstrap route seams | Replaced/shared | `WebServerJsonSeams` now owns reusable non-empty text and TCP-port body validation for server creation, server updates, server.met/nodes.dat URL imports, and Kad bootstrap requests. Command handling still owns live server lookup, priority vocabulary mapping, persistence, and Kad side effects. Tests cover bad ports, empty text fields, boolean shape, empty URL imports, and smoke-level `400` responses. |
| Preferences payload validation | Native `/api/v1/app/preferences` route seam and preference command handling | Replaced/shared | `WebServerJsonSeams::ValidatePreferencesPatchBody` validates mutable preference names, numeric preference bounds, boolean preference shape, and empty PATCH rejection before dispatch. Command handling remains responsible for applying and persisting the validated values through the normal preference save path. Tests cover empty patches, bad upload-slot bounds, bad boolean values, and smoke-level `400` responses. |
| Search-start payload validation | Native `/api/v1/searches` route seam and search-start command seam | Replaced/shared | `WebServerJsonSeams::ValidateSearchStartBody` now owns public search query, method/type vocabulary, range, availability, extension, and `clearExisting` body validation before dispatch. `WebApiCommandSeams` consumes the same default method token and vocabulary helpers before mapping to legacy search enums. Tests cover route-level bad method/type/range/boolean payloads and smoke-level `400` responses. |
| Category and friend payload validation | Native category create/update and friend-create route seams | Replaced/shared | `WebServerJsonSeams::ValidateCategoryCreateBody`, `ValidateCategoryPatchBody`, and `ValidateFriendCreateBody` now own public body shape checks for required names, empty category patches, path text, comment/color/priority fields, lowercase friend hashes, and friend-name type/safety before dispatch. Command handling still owns live category path shareability, category existence, and friend-list side effects. Tests cover route-level empty and malformed category/friend bodies plus smoke-level `400` responses. |
