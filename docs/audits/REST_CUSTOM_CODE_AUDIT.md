...
| qBit form parsing | `WebServerQBitCompatSeams::TryParseFormBody` and qBit category request parsers | Kept with reason | qBittorrent compatibility needs `application/x-www-form-urlencoded` decoding with unique decoded field names and adapter-specific error text. The implementation delegates to the shared REST URL-encoded field parser and then routes category add/set/create requests through the native category-name policy; tests cover duplicate fields, empty names, malformed escapes, required field checks, trimming, and control-character rejection. |
