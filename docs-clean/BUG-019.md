...
**Status:** In progress on `fix/bug019-aich-sync-hardening`

The current branch ports all four stabilization facets as separate sequential commits:

- `7958fda` `BUG-019 tolerate duplicate stored AICH hashes`
- `75e53d5` `BUG-019 normalize incomplete AICH tree nodes`
- `5330505` `BUG-019 harden AICH sync maintenance waits`
- `b84ee6e` `BUG-019 queue client and partfile display refreshes`
...
Regression coverage added on the branch:

- `15363c3` `BUG-019 extend display refresh seam coverage`

## Porting Note

The stale experimental branch remains the reference source for the original fixes, but the
current `fix/bug019-aich-sync-hardening` branch is the active port. Preserve the staged
commit structure when merging to `main`: isolated storage/tree-healing first, bounded sync
wait second, queued display refresh handoff last.
