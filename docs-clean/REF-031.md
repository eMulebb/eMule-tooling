...
Decision: treat this as a `FEAT-023` policy question, not as a stock-score bug. If the
feature intent is "strict seeder bias," current `main` may be acceptable. If the intent is
"same stale broadband behavior, cleaned up," the bonus placement should move back into the
float pipeline.
...
### Retune or decide explicitly under FEAT-023

- low-ratio bonus placement

`FEAT-023` should choose one of these two explicit semantics:

1. **Keep current main behavior**
   - bonus is a flat integer post-processing bump
   - strongest possible interpretation of low-ratio seeding bias
2. **Restore stale behavior**
   - bonus participates in the float score pipeline before divisor/penalties
   - better preserves proportional scoring semantics
