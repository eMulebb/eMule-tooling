...
| `CI-011` | In Progress | `7754d16` app; `13f1487`, `1058ac2`, `2bb7fd2` tests | Shared-directories REST live E2E now covers an over-`MAX_PATH` Unicode root, native `/api/v1/shared-directories` mutation, `/api/v1/shared-files` listing, category incoming-path echo/delete, persistence, relaunch reload, malformed shared-directory PATCH errors, and missing-parent root handling. Focused Release x64 `live-e2e -LiveSuite shared-directories-rest` passed in `repos\eMule-build-tests\reports\shared-directories-rest\20260506-204215-eMule-main-release`; the report records a 325-character root, `unicode-U+00DF-U+6F22.txt` fixture evidence, 400 JSON errors for blank paths and non-boolean recursive flags, a long Unicode category incoming path echoed through `/api/v1/categories`, and a missing-parent root reported as `accessible=false` with no shared files before clearing. App cleanup `7754d16` removed adapter-local CString/std::string conversion duplicates by exporting and reusing native REST conversion helpers in qBittorrent and Torznab compatibility code. Release x64 app build plus REST live E2E passed after that cleanup in `workspaces\v0.72a\state\build-logs\20260506-203443`, `repos\eMule-build-tests\reports\rest-api-smoke\20260506-203524-eMule-main-release`, and aggregate `repos\eMule-build-tests\reports\live-e2e-suite\20260506-203523-eMule-main-release\result.json`. Full Release x64 umbrella E2E remains open. |
