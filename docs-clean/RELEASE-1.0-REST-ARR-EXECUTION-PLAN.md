...
| `CI-011` | In Progress | `7754d16` app; `13f1487`, `1058ac2` tests | Shared-directories REST live E2E now covers an over-`MAX_PATH` Unicode root, native `/api/v1/shared-directories` mutation, `/api/v1/shared-files` listing, persistence, relaunch reload, malformed shared-directory PATCH errors, and missing-parent root handling. Focused Release x64 `live-e2e -LiveSuite shared-directories-rest` passed in `repos\eMule-build-tests\reports\shared-directories-rest\20260506-202644-eMule-main-release`; the report records a 325-character root, `unicode-U+00DF-U+6F22.txt` fixture evidence, 400 JSON errors for blank paths and non-boolean recursive flags, and a missing-parent root reported as `accessible=false` with no shared files before clearing. App cleanup `7754d16` removed adapter-local CString/std::string conversion duplicates by exporting and reusing native REST conversion helpers in qBittorrent and Torznab compatibility code. Full Release x64 umbrella E2E remains open. |
...
- [x] Add live E2E coverage using existing long-path shared roots where
      possible.
      - Evidence: `repos\eMule-build-tests\reports\shared-directories-rest\20260506-202644-eMule-main-release\result.json`
