...
|----|--------|----------|-------|
| `BUG-075` | In Progress | `fcedfe3`, `c8e6609`, `1e2ff57`, `69d9262` app; `28f17db`, `83093a6`, `c10f2a8`, `8e67131`, `b7406d9`, `957c31a` tests | Explicit destructive REST confirmations, content-type seam coverage, centralized native error envelope, method/API-key failure classification, direct route-failure seam coverage, and runtime-style error envelope coverage for missing objects, invalid state, service unavailable, and internal command failure classes landed. Live JSON failure smoke and adapter-specific response checks remain open. |
