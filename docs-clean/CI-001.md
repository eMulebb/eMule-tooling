---
id: CI-001
title: CMake migration — replace emule.vcxproj with CMakeLists.txt + Ninja
status: Open
priority: Major
category: ci
labels: [cmake, build-system, clangd, static-analysis]
milestone: ~
created: 2026-04-08
source: AUDIT-CODEQUALITY.md (PLAN_005 / Phase 1)
---

## Summary

Everything downstream — clang-tidy, clangd, scan-build, cppcheck with compile
commands — depends on `compile_commands.json`. CMake + Ninja generates this.
This phase replaces `emule.vcxproj` / `.sln` entirely.

**Prerequisite for:** CI-002 (clang-format), CI-003 (MSVC hardening), CI-004
(clang-tidy), CI-005 (cppcheck), CI-006 (ASan).

## Current State

- Build system: `emule.vcxproj` (MSBuild, Visual Studio 2022)
- Compiler: MSVC v143, x64 primary
- C++ standard: `C++17` (set in commit `93797f3`)
- Warning level: `/Wall`
- No `compile_commands.json` → clang-tidy/clangd cannot work

## Tools Required

```powershell
winget install Ninja-build.Ninja   # generates compile_commands.json
# Already installed:
# - LLVM 22.1.0 at C:\Program Files\LLVM\bin\
# - CMake 4.2.0 at C:\Program Files\CMake\bin\
# - MSVC v143 (VS 2022 Professional)
```

## Target Directory Layout

```
eMule-main/
├── srchybrid/
│   ├── CMakeLists.txt     ← defines eMule target
│   └── stdafx.h / stdafx.cpp
├── CMakeLists.txt          ← workspace root, wires deps
└── CMakePresets.json       ← Debug/Release x64 presets
```

## Key CMake Requirements

```cmake
cmake_minimum_required(VERSION 3.25)
set(CMAKE_CXX_STANDARD 17)
set(CMAKE_CXX_STANDARD_REQUIRED ON)
set(CMAKE_CXX_EXTENSIONS OFF)
set(CMAKE_EXPORT_COMPILE_COMMANDS ON)      # enables clang-tidy/clangd
set(CMAKE_MSVC_RUNTIME_LIBRARY "MultiThreaded$<$<CONFIG:Debug>:Debug>")
```

## Dependency Root Variables (CMakePresets.json)

```json
"cacheVariables": {
  "CRYPTOPP_ROOT":  "${sourceDir}/../eMule-cryptopp",
  "MINIUPNP_ROOT":  "${sourceDir}/../eMule-miniupnp",
  "RESIZABLE_ROOT": "${sourceDir}/../eMule-ResizableLib",
  "ZLIB_ROOT":      "${sourceDir}/../eMule-zlib"
}
```

## Acceptance Criteria

- [ ] `cmake --preset Debug` produces a working `eMule.exe`
- [ ] `compile_commands.json` generated in the build directory
- [ ] VS Code `cmake-tools` + `clangd` extension can build and provide
      IntelliSense from the CMake project
- [ ] Both x64 Debug and x64 Release configurations pass
- [ ] ARM64 configuration passes (or is documented as deferred)
- [ ] `emule.vcxproj` is removed from the repository

## Files

- New: `srchybrid/CMakeLists.txt`
- New: `CMakeLists.txt` (workspace root)
- New: `CMakePresets.json`
- Delete: `emule.vcxproj`, `emule.sln`
