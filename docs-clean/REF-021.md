...
The backlog schema does not have a dedicated `Deferred` status, so this item is
tracked as **Blocked** to preserve the decision in the active backlog.

Current rationale:

- the remaining deprecated-Winsock usage is real but low practical impact for
  the current branch
...
This item remains valid backlog work, but it should stay deferred unless the
warning-floor reduction effort or networking modernization work is explicitly
reprioritized.
...
- high correctness value: pointer/handle truncation, integer narrowing,
  implicit fallthroughs, non-portable varargs, throwing C/Win32 callbacks, and
  hidden virtual overrides
- medium value: numeric precision warnings in transfer, scheduling, scoring,
  rate-limit, and persisted-value code
- low value unless adjacent code is already being changed: `register`
  cleanup, unused parameters, padding/layout noise, inline decisions, and
