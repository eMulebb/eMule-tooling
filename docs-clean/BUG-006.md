...
**Current status: Rejected for this branch.** Kept open for future branch as the
correct fix is straightforward: replace `rand() + 1` with `GetRandomUInt32()`.

## Correct Fix (for future branch)
